...
pub fn drop_row(grid: Grid(a), y: Int) -> Grid(a) {
  case dict.get(grid, y) {
    Ok(_) ->
      dict.delete(grid, y)
      |> drop_reindex_rows(y + 1)
    Error(_) -> grid
  }
}
...
pub fn slice_column(grid: Grid(a), x: Int) -> Result(List(a), Nil) {
  slice_column_row(grid, x, 0)
}

pub fn slice_row(grid: Grid(a), y: Int) -> Result(Row(a), Nil) {
  dict.get(grid, y)
}
...
pub fn size(grid: Grid(a)) -> #(Int, Int) {
  let height = dict.size(grid)

  case dict.get(grid, 0) {
    Ok(row) -> #(height, dict.size(row))
    Error(_) -> #(height, 0)
  }
}
...
fn drop_reindex_rows(grid: Grid(a), y: Int) -> Grid(a) {
  case dict.get(grid, y) {
    Ok(row) ->
      dict.delete(grid, y)
      |> dict.insert(y - 1, row)
...
fn slice_column_row(grid: Grid(a), x: Int, y: Int) -> Result(List(a), Nil) {
  case dict.get(grid, y) {
    Ok(row) ->
      case dict.get(row, x), slice_column_row(grid, x, y + 1) {
